...
def create_confirmed_reservation(*, room_id, guest_name, guest_email, check_in, check_out, guests, customer_user=None):
    """Lock the room, recheck the dates, and create the reservation atomically."""
    with transaction.atomic():
        room = Room.objects.select_for_update().get(pk=room_id)
        if not room.is_available or guests > room.capacity or Reservation.overlaps_room(room.id, check_in, check_out):
        ...
        reservation = Reservation.objects.create(
            ...
            guests=guests,
            status='confirmed',
            total_price=room.base_price_per_night * (check_out - check_in).days,
        )
    return reservation
